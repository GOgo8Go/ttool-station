...
const UnitConverter: React.FC = () => {
    ...
    const convert = () => {
        ...
        if (category === 'temperature') {
            ...
            const rates = UNIT_CATEGORIES[category].units;
    ...
    const generateConversionTable = () => {
        if (!UNIT_CATEGORIES[category] || !UNIT_CATEGORIES[category].units) return [];

        const units = Object.keys(UNIT_CATEGORIES[category].units);
        const baseUnit = UNIT_CATEGORIES[category].base;
        const baseRate = UNIT_CATEGORIES[category].units[baseUnit];
        ...
        return units.map(unit => {
            ...
            const rate = UNIT_CATEGORIES[category].units[unit];
                                    ...
                                    <div className="absolute z-10 mt-1 w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg max-h-60 overflow-auto">
                                        <div className="grid grid-cols-2 gap-4 p-3">
                                            ...
                                            <div>
                                                ...
                                                {UNIT_CATEGORIES[category] && Object.keys(UNIT_CATEGORIES[category].units)
                                ...
                                {fromDropdownOpen && (
                                    <div className="absolute z-10 mt-1 w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg max-h-60 overflow-auto">
                                        <div className="grid grid-cols-3 gap-1 p-2">
                                            {UNIT_CATEGORIES[category] && Object.keys(UNIT_CATEGORIES[category].units).map(u => (
                                                <button
                                                    key={u}
                                                    type="button"
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead className="bg-gray-50 dark:bg-gray-900">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                    {t('tool.unit-converter.value')} (1 {t(`tool.unit-converter.units.${UNIT_CATEGORIES[category].base}`)} = ?)
                                </th>
                            </tr>
                        </thead>
